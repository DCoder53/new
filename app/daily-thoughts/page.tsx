...
const DailyPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDailyMessages = async () => {
      try {
        const response = await axios.get<Message[]>('http://localhost:5000/api/messages/daily');
        setMessages(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching daily messages');
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Thoughts from the Last 24 Hours</h1>
